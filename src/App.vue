    ...
    <div class="bg-zinc-200 rounded-full mx-4 my-4 py-2">
      <div as="div" class="relative inline-block flex flex-row mx-2 items-center">
        <div class="text-xs px-3">
          ...
          <span v-if="isLogin" class="text-black/50">/ {{workTime}}</span>
        </div>
        ...
        <img el_menu
          ...
          @click.self="OpenMenu"
        >
        <div v-if="isLogin" class="absolute bg-green-500 opacity-90 h-2 w-2 rounded-full top-2 -right-1" />
         </div>

        

        <div  @click.self="OpenMenu"  el_menu
        class="cursor-pointer inline-flex rounded-full bg-zinc-200 px-4 py-0 text-sm font-bold text-black/75 hover:bg-opacity-30 hover:text-black/100  focus:outline-none focus-visible:ring-2 focus-visible:ring-white focus-visible:ring-opacity-75">
          {{userName}}
          <ChevronDownIcon @click.self="OpenMenu" el_menu
         ...
         <myMenu :active="MenuActive" :items="arMenuItems" @click="SelMenuItem"/>
          ...
          <!-- <button
            @click="Entrar()"
            type="button"
            class="rounded-full bg-teal-400 bg-opacity-60 px-2 py-1 mx-2 text-xs text-white w-28 hover:bg-opacity-90 focus:outline-none focus-visible:ring-2 focus-visible:ring-white focus-visible:ring-opacity-75"
          >
            Entrar
          </button>
          ...
          <button
            @click="Salir()"
            type="button"
            class="rounded-full bg-red-400 bg-opacity-80 px-2 py-1 mx-2 text-xs text-white w-28 hover:bg-opacity-100 focus:outline-none focus-visible:ring-2 focus-visible:ring-white focus-visible:ring-opacity-75"
          >
            Salir
          </button>     
          ...
          <div class="text-xs px-3">
          <span class="text-black/75">{{workTime}}</span>
          <span class="text-black/50">/ {{workTime}}</span>
          </div>      -->
    ...
    {id: 1,
     ...
     [
       { 
         ...
         id: 101,
         text:'<strong>Sesame1</strong> Kelli Pierce<br><div class="">Hoy llevas 00:00</div> ',
         icon: 'ArchiveIcon',
         cmd:"session1",
         iconText:"S1",
         SessionName: "Sesame1",
       ...
       { 
         id: 102,
         text:'<strong>Sesame2</strong> Kelli Pierce<br><div class="">Hoy llevas 00:00</div> ',
         icon: 'ArchiveIcon',
         cmd:"session2",
         iconText:"S2",
         SessionName: "Sesame2",
...
// Geolocation
let postion = { latitude: 0, longitude: 0 };
...
const geo_success = (pos) => {
  postion.latitude = pos.coords.latitude;
  postion.longitude = pos.coords.longitude;
};
...
document.addEventListener("click", (event) =>{
  if (!event.target.hasAttribute("el_menu") && event.target.tagName!='svg' ) MenuActive.value=false;
});
